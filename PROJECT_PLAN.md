...
- [x] 原生端在应用沙盒保存 `global.memory.md`；Web 使用浏览器本地存储保存相同 Markdown 文档。
- [x] 设置 → 数据提供总开关和记忆管理页；默认关闭，用户主动开启。
- [x] 用户可从聊天消息手动“记住”，保存前允许整理文本；支持新增、编辑、常驻/按需和删除。
- [x] 每次请求最多召回 8 条、4000 字符；当前用户消息优先于旧记忆。
- [x] 写入前拦截 API Key、Bearer Token、密码、私钥和 URL 凭证。
- [x] 写入串行化、临时文件原子替换并保留上一版 `.bak`。
- [x] Markdown 编解码、去重、召回、上下文注入和手机窄屏管理页测试。
- [x] 普通会话支持按需调用聊天模型整理最多 5 条候选记忆；本地校验用户消息证据、置信度、敏感信息和重复项，手机端逐条勾选后才写入。
- [x] 候选可标记为新增、更新或冲突；确认页并排展示新旧内容，支持原子替换、两条都保留或暂不保存。
- [x] 支持 Markdown 备份导出与安全导入：手机走系统分享，导入离线预检新增/重复/不同版本，敏感或异常条目拦截后一次性原子合并。
- [ ] 后续：临时记忆有效期、时间线和语义向量索引。

### ✅ M9 — 系统语音输入（首版已完成）

- [x] 输入框提供独立麦克风按钮，首次点击时才请求麦克风/语音识别权限。
- [x] 默认优先中文系统识别，支持临时结果，识别文字插入当前光标位置并保留原草稿。
- [x] 识别结果只进入输入框，不自动发送；监听期间有明确状态，停止后可以继续编辑。
- [x] 发送、切换会话、应用退到后台或页面销毁时取消监听，不在应用中保存原始录音。
- [x] Android/iOS/macOS 平台权限已配置；不可用、拒绝权限、网络和超时错误提供中文提示。
- [x] 已覆盖文字合并、拒绝权限、不自动发送和手机窄屏布局测试。
- [ ] Android/iOS 真机确认不同厂商系统识别服务的授权、断句和自动停止行为。
