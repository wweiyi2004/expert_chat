...
质量优先（文风以用户选择和故事情节为准；未指定时使用自然、克制的现代中文叙事）：
1. 忠于用户「故事情节」与硬性约束：不得改结局/基调，不得用“更戏剧”绕过禁忌。
2. outline 每一拍必须是**可拍的一场戏**（谁、在哪、发生了什么、导致什么），包含互动或行动契机；禁止“情感升温/关系发展”这类空拍，也禁止把多章信息量塞进一拍。
3. 角色 1–6 人：彼此有明确冲突或牵绊；personality 必须写清语气、动机与可辨特征，exampleDialogs 应体现差异；不要同质化龙套。
4. 第一拍必须能立刻开场（日常或气氛 + 细微异常钩子），最后一拍必须收束用户要求的结局（若用户指定了结局）。
5. authorNote 只写方案中新产生、且需要长期保持的导演备注；不要复述用户原文约束，也不要重复“AI 演全员、用户是导演”等基础模式规则。没有新增备注时返回空字符串。
